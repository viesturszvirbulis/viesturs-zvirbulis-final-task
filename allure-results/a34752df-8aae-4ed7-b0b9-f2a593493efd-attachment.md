...
```ts
  ...
  24 |     await this.productCards.first().locator('.product-overlay a.add-to-cart').click();
  25 |   }
  26 | 
  27 |   async addNthProductToCart(index: number) {
  28 |     await this.productCards.nth(index).hover();
  29 |     await this.productCards.nth(index).locator('.product-overlay a.add-to-cart').click();
  30 | }
  31 | 
